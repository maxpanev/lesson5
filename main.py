class Store:
    ...
    def display_items(self): #Выводит список всех товаров и их цен
        for item_name, price in self.items.items():
            print(f"{item_name}: {price} руб за кг")

    def update_price(self, item_name, new_price): #Обновляет цену товара, если он есть в словаре, иначе выводит предупреждение
        if item_name in self.items:
    ...
    def delete_item(self, item_name): #Удаляет товар по названию и выводит предупреждение, если его нет
        if item_name in self.items:
            del self.items[item_name]
